...
def create_site_in_bench(args):
    if "mariadb" == args.db_type:
        ...
        new_site_cmd = [
            ...
            f"--no-mariadb-socket",
            f"--db-root-password={os.getenv('MYSQL_ROOT_PASSWORD','123')}",  # Replace with your MariaDB password
        ...
        new_site_cmd = [
            ...
            f"--db-type={args.db_type}",  # Add the selected database type
            f"--db-root-password={os.getenv('MYSQL_ROOT_PASSWORD','123')}",  # Replace with your PostgreSQL password
            f"--admin-password={args.admin_password}",
        ]
